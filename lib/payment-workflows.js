...
import { createAdjustedTransfer, isPermanentTransferError, buildBulkTransferAmounts, round2 } from './transfer-fees.js';
...
async function createTransfersForAmount(opts) {
  ...
  const chunks =
    useBatch && Number(batchPerAmount) > 0
      ? buildBulkTransferAmounts(remaining, Number(batchPerAmount))
      : [remaining];
