...
import { createAdjustedTransfer, isPermanentTransferError, buildFeeAwareBulkTransferAmounts, estimateDebitFromGross, maxGrossForDebitBudget, getFeePct, round2 } from './transfer-fees.js';
import { paymentHasExistingTransfer, sumSentGrossForPaymentDestination } from './transfer-lookup.js';
...
function ruleUsesBatch(rule) {
  return Boolean(rule?.batch_enabled) && Number(rule?.batch_per_amount) > 0;
}

async function createTransfersForAmount(opts) {
  const {
    whop,
    userId,
    companyId,
  ...
  if (!Number.isFinite(target) || target <= 0) {
    return { transfersCreated, errors, complete: true };
  }

  const useBatch = batchEnabled && Number(batchPerAmount) > 0;
  let alreadySent = 0;
  if (useBatch || target > 0) {
    alreadySent = await sumSentGrossForPaymentDestination(
      whop,
      companyId,
      paymentId,
      destinationId,
  ...
  const remaining = round2(target - alreadySent);
  if (remaining <= 0.001) {
    return { transfersCreated, errors, complete: true };
  }
  ...
  let commissionPct = 1;
  let feePct = 0.03;
  if (useBatch && Number(batchPerAmount) > 0) {
    const settings = await db.getUserSettings(userId);
    commissionPct = Number(settings?.platform_commission_pct ?? 1);
    feePct = await getFeePct(userId);
    chunks = buildFeeAwareBulkTransferAmounts(remaining, Number(batchPerAmount), commissionPct, feePct);
  } else {
    chunks = [remaining];
  }

  let debitRemaining = remaining;
  for (let i = 0; i < chunks.length; i++) {
    let gross = chunks[i];
    const isLast = i === chunks.length - 1;
    if (isLast && useBatch) {
      const capped = maxGrossForDebitBudget(debitRemaining, commissionPct, feePct);
    ...
    try {
      ...
      const actualDebit =
        result.sendable != null
          ? round2(Number(result.sendable))
          : estimateDebitFromGross(result.gross ?? gross, commissionPct, feePct);
      ...
      transfersCreated.push({
        ...
        batch_index: useBatch ? i + 1 : undefined,
        batch_total: useBatch ? chunks.length : undefined,
      });
    } catch (e) {
  ...
  const sentThisRun = transfersCreated.reduce((sum, t) => sum + (Number(t.gross) || 0), 0);
  const totalSent = round2(alreadySent + sentThisRun);
  const complete = errors.length === 0 && totalSent >= target - 0.01;

  return { transfersCreated, errors, complete };
}
...
export async function runSplitForPayment(paymentId, userId, ctx = {}) {
  ...
  for (const split of rule.splits || []) {
    ...
    const batch = {
      batchEnabled: rule.batch_enabled,
      batchPerAmount: rule.batch_per_amount,
    };
    const result = await createTransfersForAmount({
      whop,
      userId,
      companyId,
      paymentId,
      ...
      currency,
      ...batch,
      extraMetadata: { percentage: pct },
    });
    for (const t of result.transfersCreated) {
      transfersCreated.push({ ...t, percentage: pct });
    }
    errors.push(...result.errors);
    if (!result.complete) allComplete = false;
  }

  if (allComplete && errors.length === 0) {
  ...
  return { ran: true, ruleId: rule.id, transfersCreated, errors, batch: ruleUsesBatch(rule), complete: allComplete && errors.length === 0 };
}
...
export async function runAutoTransferForPayment(paymentId, userId, ctx = {}) {
  ...
  if (destId) {
    ...
    if (!Number.isFinite(gross) || gross <= 0) {
      ...
      for (const t of result.transfersCreated) {
        transfersCreated.push({ ...t, rule_id: rule.id });
      }
      for (const e of result.errors) {
        errors.push(e);
        console.error(
          `[auto-transfer] Failed payment=${paymentId} dest=${destId}: ${e.message}`
        );
      }
    }
  }

  if (workflowComplete && errors.length === 0) {
  ...
  return {
    ...
    transfersCreated,
    errors,
    batch: ruleUsesBatch(rule),
    complete: workflowComplete && errors.length === 0,
  };
}
...
export function evaluatePaymentJobOutcome(result) {
  ...
  const batchIncomplete = parts.some((p) => p.ran && p.batch && p.complete === false);
  if (batchIncomplete) {
    return {
      shouldComplete: false,
      shouldRequeue: true,
      lastError: 'Batch transfer incomplete — will resume remaining chunks on retry',
    };
  }
      ...
      if (loaded.error) {
        ...
        const transferRule =
          transferState.enabled && !transferState.processedPaymentIds.includes(paymentId)
            ...
            : null;
        const batchWorkflow =
          ruleUsesBatch(splitRule) || ruleUsesBatch(transferRule);

        if (alreadyOnWhop && !batchWorkflow) {
          if (
            splitState.enabled &&
