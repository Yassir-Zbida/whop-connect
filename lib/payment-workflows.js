...
const MIN_TRANSFER_USD = Number(process.env.WHOP_MIN_TRANSFER_AMOUNT) || 1;
...
export async function runSplitForPayment(paymentId, userId, ctx = {}) {
  ...
  for (const split of rule.splits || []) {
    ...
    if (splitAmount < MIN_TRANSFER_USD) {
      errors.push({
        destination_id: destId,
        message: `Split amount ${splitAmount} below minimum ${MIN_TRANSFER_USD}`,
      });
      continue;
    }
    try {
...
export async function runAutoTransferForPayment(paymentId, userId, ctx = {}) {
  ...
  if (destId) {
    ...
    if (!Number.isFinite(transferAmount) || transferAmount < MIN_TRANSFER_USD) {
      errors.push({
        destination_id: destId,
        rule_id: rule.id,
        message: `Transfer amount ${transferAmount} below minimum ${MIN_TRANSFER_USD}`,
      });
    } else {
      try {
        const transfer = await whop.transfers.create({
...
export function evaluatePaymentJobOutcome(result) {
  ...
  if (transferErrors.length > 0) {
    return {
      ...
      shouldRequeue: true,
      lastError: transferErrors.map((e) => e.message || String(e)).join('; ').slice(0, 2000),
    };
  }
